...
const MobileNavbar = () => {
  ...
  return (
    <div className="absolute w-full start-0 top-14 h-screen bg-primary overflow-y-auto ">
      {navLinks.map((nav) => (
        <div key={nav.label} className="border-b border-lightgray px-10 ">
          <button
            onClick={() => toggleDropdown(nav.label)}
            className="text-white-100 flex justify-between items-center  w-full gap-4 my-5 py-2 px-2 translate-y-0 transition-all duration-300"
          >
          ...
          <div
            className={`overflow-hidden transition-max-height duration-300 ease-in-out ${
              openDropdown === nav.label ? "max-h-screen" : "max-h-0"
            }`}
          >
